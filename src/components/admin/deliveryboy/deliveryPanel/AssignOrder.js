...
const AssignOrder = () => {
  ...
  const {
    ...
    ["get_assign_order", { searchTerm, startDate, endDate, page, statusType }],
    () =>
  ...
  const handleViewLocation = async (orderUnique) => {
    try {
      ...
      const res = await apiConnectorGet(
        `${endpoint.get_order_location}?order_id=${orderUnique}`
      );
  ...
  }, [searchTerm, startDate, endDate, statusType]);
  ...
  const updateOrderStatus = async (orderId, status) => {
    ...
    try {
      ...
      const res = await apiConnectorPost(
        endpoint.update_cust_order_status,
        payload
      );
      toast(res?.data?.message);
      if (res?.data?.success) {
  ...
  const SubmitLocationUpdate = async () => {
    ...
    try {
      ...
      const res = await apiConnectorPost(
        endpoint.update_order_location,
        payload
      );
      toast(res?.data?.message);
      if (res?.data?.success) {
      ...
      <div className="flex gap-2" key={`actions-${order.otd_id}`}>
        {order?.otd_status === "Pending" ? (
          <>
            <button
              onClick={() =>
                handleOrderStatusChange(order.otd_id, order.order_unique, 2)
              }
      ...
      {rejectModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg w-full max-w-md space-y-4">
            <h2 className="text-xl font-semibold text-red-600">Reject Order</h2>
            <textarea
              rows="4"
                    ...
                    <StepContent>
                      ...
                      <p className="text-sm text-gray-700">
                        ...
                        <span className="font-extrabold">
                          {loc?.ol_last_dist ?? "N/A"} Km
                        </span>
                      </p>
                      <p className="text-sm text-gray-700">
                        <strong>Updated At:</strong>{" "}
                        {loc?.ol_created_at
                          ? new Date(loc.ol_created_at).toLocaleString(
                              "en-IN",
                              {
                                dateStyle: "medium",
                                timeStyle: "short",
