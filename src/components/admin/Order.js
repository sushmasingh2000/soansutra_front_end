...
const Order = () => {
    ...
    const fetchOrders = async () => {
        try {
            setLoading(true);
            const res = await apiConnectorGet(`${endpoint.get_order}?page=${page}&count=${10}`);
    ...
    const handleStatusChange = async (orderId, status) => {
        const confirmChange = window.confirm(`Are you sure you want to mark this order as ${status}?`);
                        ...
                        {orders?.data?.length > 0 ? (
                            orders?.data?.map((order, index) => (
                                <tr key={order.order_id} className="border-t hover:bg-gray-50">
                                    ...
                                    <td className="px-4 py-2">{order.order_unique}</td>
                                    ...
                                    <td className="px-4 py-2">
                                        <div className="flex flex-wrap gap-2">
                                            {statusButtons.map(({ status, bgColor, hoverColor }) => {
                                                const currentIndex = statusButtons.findIndex((btn) => btn.status === order.status);
                                                const buttonIndex = statusButtons.findIndex((btn) => btn.status === status);
                                                ...
                                                return (
                                                    <button
                                                        key={status}
                                                        onClick={() => !isDisabled && handleStatusChange(order.order_unique, status)}
                                                        disabled={isDisabled}
                                                        className={`
