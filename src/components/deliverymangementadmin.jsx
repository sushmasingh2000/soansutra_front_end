...
const DeliveryAdminPanel = ({
  ...
  const confirmAssign = () => {
    ...
    addNotification(`Order #${selectedOrder.id} assigned to ${updatedOrder.assignee}`);
  ...
  const openEditBoyModal = (boyId) => {
    ...
    setEditBoyAvailability(boy.available);
  ...
  const saveEditBoy = () => {
    ...
    const updatedBoy = {
      ...selectedBoy,
      name: editBoyName,
      available: editBoyAvailability
    };
    updateBoy(updatedBoy);
    alert("Delivery boy updated!");
    setShowEditBoyModal(false);
  };

  const handleAddBoy = () => {
    addBoy(boyName, boyAvailability);
    setBoyName('');
    alert("Delivery boy added!");
  };

  const handleDeleteBoy = (boyId) => {
  ...
  return (
    <div id="adminPanel" className="block">
      ...
      {activeTab === 'orders' && (
        <div id="adminOrders" className="block">
            ...
            {filteredOrders.map(order => (
              ...
              >
                <div className="flex">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium">
                      ...
                      <span className={`ml-2 px-2 py-1 rounded text-white text-xs ${order.distance <= 3 ? "bg-green-500" : "bg-red-500"}`}>
                        {order.distance <= 3 ? "Local" : "Third-Party"}
                      </span>
                    </p>
                    <p className="text-sm font-medium">Status: {order.status}</p>
                    {order.assignee && (
                      <p className="text-sm text-gray-600">Assigned to: {order.assignee}{order.awb ? ` (AWB: ${order.awb})` : ''}</p>
                    )}
                  </div>
                </div>
                <div className="mt-4 flex space-x-2">
                  ...
                  ) : order.assignee.startsWith("Third-Party:") ? (
                    <select
                ...
                <div className="mt-2 flex space-x-2 timeline">
                  {statuses.map((s, i) => (
                    <div
                      key={s}
                      className={`${order.status === s || i < statuses.indexOf(order.status) ? "bg-yellow-500" : "bg-gray-200"} h-2 rounded`}
                      style={{ flex: 1 }}
                    />
                  ))}
                </div>
      ...
      {activeTab === 'boys' && (
        <div id="adminBoys" className="block">
          ...
          <div className="mb-4">
            ...
            <select
              ...
              onChange={(e) => setBoyAvailability(e.target.value === 'true')}
            >
              <option value="true">Available</option>
              <option value="false">Busy</option>
            </select>
            <button onClick={handleAddBoy} className="px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600">
              Add Boy
            </button>
          </div>
            ...
            {deliveryBoys.map(boy => (
              <div key={boy.id} className="bg-white p-4 rounded-lg shadow flex justify-between items-center">
                ...
                <div className="space-x-2">
                  <button onClick={() => openEditBoyModal(boy.id)} className="px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600">
                    Edit
                  </button>
                  <button onClick={() => handleDeleteBoy(boy.id)} className="px-3 py-1 bg-red-500 text-white rounded hover:bg-red-600">
                    Delete
                  </button>
                </div>
              </div>
      ...
      {showAssignModal && selectedOrder && (
        <div className="modal active">
          <div className="bg-white p-6 rounded-lg w-96">
            <h2 className="text-xl font-bold mb-4">Assign Order #{selectedOrder.id}</h2>
            <p className="text-sm mb-2">Distance: {selectedOrder.distance}km</p>
            <p className="text-sm mb-4">Suggestion: {selectedOrder.distance <= 3 ? "Local Boy" : "Third-Party"}</p>
            <div className="mb-4">
              <label className="block text-sm font-medium mb-1">Assign To:</label>
              <select
                className="border p-2 rounded w-full"
                value={localBoy}
                onChange={(e) => { setLocalBoy(e.target.value); setThirdParty(''); setAwbNumber(''); }}
              >
                <option value="">Select Local Boy</option>
                {deliveryBoys.map(boy => (
                  <option key={boy.id} value={boy.name}>{boy.name} ({boy.available ? "Available" : "Busy"})</option>
                ))}
              </select>
            </div>
            <div className="mb-4">
              ...
              <select
                className="border p-2 rounded w-full"
                value={thirdParty}
                onChange={(e) => { setThirdParty(e.target.value); setLocalBoy(''); }}
              >
            ...
            <div className="flex justify-end space-x-2">
              ...
              <button onClick={confirmAssign} className="px-4 py-2 bg-yellow-500 text-white rounded hover:bg-yellow-600">Confirm</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white p-6 rounded-lg w-96">
            ...
            <div className="mb-4">
              ...
              <select
                ...
                onChange={(e) => setEditBoyAvailability(e.target.value === 'true')}
              >
                <option value="true">Available</option>
                <option value="false">Busy</option>
              </select>
            ...
            <div className="flex justify-end space-x-2">
              <button onClick={() => setShowEditBoyModal(false)} className="px-4 py-2 bg-gray-200 rounded">Cancel</button>
              <button onClick={saveEditBoy} className="px-4 py-2 bg-yellow-500 text-white rounded hover:bg-yellow-600">Save</button>
            </div>
          </div>
        </div>
      )}
