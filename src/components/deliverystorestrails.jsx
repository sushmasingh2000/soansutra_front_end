import { Check, Clock, MapPin, Store, Truck } from "lucide-react";
...
const DeliveryStoresUI = ({ selected_v }) => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [pinCode, setPinCode] = useState("000000"); // State to store pin code
  ...
  async function getNearestStore(inv_id) {
    setLoding(true);
    if (pinCode !== "000000") {
      const latlong = await getLatLongByPincode(pinCode);
      const storeData = await apiConnectorPost(endpoint.get_nearest_store, {
        long: latlong?.lng,
        lat: latlong?.lat,
        inv_id: inv_id,
      });
      if (storeData?.data?.success) {
        setNearestStore(storeData?.data?.result || []);
      }
    } else {
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          async (pos) => {
            const storeData = await apiConnectorPost(
              endpoint.get_nearest_store,
              {
  ...
  useEffect(() => {
    selected_v && getNearestStore(selected_v);
  }, [selected_v, pinCode]);

  const store_near_by = nearestStore?.find((i) => i?.is_nearest === true) || {};
  return (
      ...
      <div className="bg-white rounded-lg p-3 border border-grey-200">
        <div className="flex items-start space-x-2 mb-3">
          ...
          <div className="flex-1">
            <h3 className="text-sm font-semibold text-gray-800 mb-2">
              ...
              {loading ? (
                ...
                <span className="text-gray-700 !font-extrabold">
                  {store_near_by.name} {store_near_by.address} (
                  {store_near_by.distance?.toFixed(2)} km)
                </span>
              ) : (
                <span>No Stock Available in any store.</span>
              )}
            </h3>
              ...
              {loading ? (
                ...
                <div className="bg-green-100 px-2 py-0.5 rounded-full flex items-center space-x-1">
                  {store_near_by ? (
                    ...
                    <Cancel className="w-3 h-3 text-rose-600" />
                  )}
                  {store_near_by ? (
            ...
            {!loading && nearestStore?.length - 1 > 0 && (
              <p
              ...
              >
                ...
                <span className="text-yellow-600 font-medium">
                  {nearestStore?.length - 1} other stores
                </span>
              </p>
            )}
            {showMore && (
              <div className="overflow-x-auto">
                <table className="min-w-full text-xs text-gray-700 border border-gray-200 rounded-md">
                  <thead className="bg-gray-100 text-gray-600 uppercase text-[10px]">
                    <tr>
                      ...
                      <th className="px-2 py-1 text-left">Distance (Km)</th>
                    </tr>
                  </thead>
                  <tbody>
                    {nearestStore
                      ?.filter((j) => !j?.is_nearest)
                      ?.map((i, idx) => (
                        <tr
                          key={idx}
                        ...
                        >
                          <td className="px-2 py-1">
                            ...
                            <span className="text-gray-500">{i?.address}</span>
                          </td>
                          <td className="px-2 py-1">{i?.pin_code}</td>
                          <td className="px-2 py-1">{i?.swi_qnty}</td>
                          <td className="px-2 py-1">
                ...
                {showMore && (
                  <p
                    onClick={() => setShowMore(!showMore)}
                    className="!text-xs text-right text-amber-700 cursor-pointer"
                  >
                    Hide
                  </p>
                )}
      ...
      <div className="bg-green-50 rounded-xl p-4 border border-green-200">
        <div className="flex items-start space-x-4">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="font-semibold text-gray-800">Live Video Call</h3>
            <p className="text-xs text-gray-600 leading-relaxed mb-2">
              Join a live video call with our consultants to see your favourite
              designs up close!
            </p>
            <button
