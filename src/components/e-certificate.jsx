...
const CertificateUI = () => {
  ...
  const { data, isLoading, isError } = useQuery(
    ...
    () =>
      apiConnectorGet(
        `${endpoint.e_certificate}?certificate_id=${certificateNo}`
      ),
    usequeryBoolean
  );
  ...
  if (isLoading) {
    return (
      <div className="text-center py-10 text-yellow-600 font-semibold">
        Loading Certificate...
      </div>
    );
  }
  ...
  const certificate = {
    ...
    height:
      rawData?.attributes?.find((a) => a.attribute_name === "Height")?.value ||
      "N/A",
    width:
      rawData?.attributes?.find((a) => a.attribute_name === "Thickness")
        ?.value || "N/A",
    grossWeight: rawData?.material_details?.[0]?.weight || "N/A",
    metalPurity: `${rawData?.pur_stamp_name || "N/A"} ${
      rawData?.master_mat_name || ""
    }`,
  ...
  return (
    <div className="min-h-screen bg-gray-100 font-sans antialiased px-2 sm:px-0 py-4">
      ...
      <div className="max-w-4xl mx-auto p-4 sm:p-6 border-4 border-yellow-600 rounded-lg shadow-lg bg-white">
        ...
        {/* Gold / Purity */}
        {/* Metal / Purity Blocks */}
        {rawData?.material_details?.length > 0 &&
          rawData.material_details.map((mat, index) => (
            <div key={index} className="mb-8 p-4 bg-yellow-50 rounded-lg">
              <h3 className="text-base font-semibold text-yellow-600 mb-2">
                {mat.master_mat_name || "Material"} / PURITY
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div>
                  <p className="font-medium">Metal / Purity:</p>
                  <p className="text-gray-700">
                    {mat.pur_stamp_name || "N/A"} {mat.master_mat_name || ""}
                  </p>
                </div>
                <div>
                  <p className="font-medium">Net Metal Weight:</p>
                  <p className="text-gray-700">
                    {mat.weight || "N/A"} {mat.ma_unit || "g"}
                  </p>
                </div>
              </div>
            </div>
...
{certificate?.metalName} 
        ...
        {rawData?.material_details?.length > 0 &&
          rawData.material_details.map((mat, index) => (
            <div key={`table-${index}`} className="mb-8">
              <h3 className="text-base font-semibold text-yellow-600 mb-4">
                {mat.master_mat_name || "Material"}
              </h3>
              <table className="w-full border-collapse border border-yellow-300 text-sm">
                <thead>
                  <tr className="bg-yellow-100">
                    <th className="border border-yellow-300 p-2 text-left">
                      QUALITY
                    </th>
                    <th className="border border-yellow-300 p-2 text-left">
                      WEIGHT ({mat.ma_unit || "g"})
                    </th>
                  </tr>
                </thead>
                <tbody>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center text-yellow-600 border-t border-yellow-300 pt-4 text-sm">
          <a
            href="https://www.sonasutra.com/certificate"
