...
const DirectCustomer = () => {
  ...
  const { data, isLoading, refetch } = useQuery(
    ["direct_customer", { searchTerm, startDate, endDate, page }],
    () =>
  ...
  const directcustomer = data?.data?.result || [];
  ...
  const tablerow = directcustomer?.data?.map((item, index) => [
  ...
  return (
    ...
    <h2 className="text-xl font-semibold mb-4">Direct Customer</h2>
     ...
     <div className=" overflow-x-scroll w-auto">
      ...
      <CustomToPagination data={directcustomer} setPage={setPage} page={page} />
    </div>
  );
};
