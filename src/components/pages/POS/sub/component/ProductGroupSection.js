...
import UpsertGroup from "../forms/UpsertGroup";
...
const ProductGroupSection = () => {
  ...
  const { data, isLoading } = useQuery(
    ["get_product_group_items", searchTerm],
    () => apiConnectorGet(endpoint?.get_groups, { search: searchTerm }),
    {
  ...
  const deleteProductGroup = async (id) => {
    try {
      ...
      const res = await apiConnectorGet(endpoint?.delete_group, reqbody);
  ...
  const tablerow =
    data?.data?.result?.data?.length > 0
      ? data?.data?.result?.data?.map((item, index) => [
          ...
          <div className="text-center w-full">{item?.pg_is_gold || "--"}</div>,
          <div className="text-center w-full">
            {item?.pg_is_silver || "--"}
          </div>,
          <div className="text-center w-full">
          ...
          <div className="text-center w-full">
            {item?.pg_created_at
              ? moment(item?.pg_created_at).format("DD-MM-YYYY")
              : "--"}
          </div>,
          <div className="text-center w-full">
            {item?.pg_updated_at
              ? moment(item?.pg_updated_at).format("DD-MM-YYYY")
              : "--"}
          </div>,
          <div className="text-center w-full !flex !gap-2">
            ...
            <Delete
              className="!text-xs"
              onClick={() =>
                areYouSureFn(Swal, deleteProductGroup, item?.pg_id)
              }
            />
          </div>,
        ])
  ...
  return (
    <>
      ...
      {selectedField === "Add" && <UpsertGroup selectedItems={selectedItems} />}
    </>
  );
};
